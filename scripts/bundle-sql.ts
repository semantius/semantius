...
/**
 * SQL Bundler - Build script for @semantius/triggerdev
 *
 * Reads SQL migration files from the apps/ directory and bundles their
 * contents into a TypeScript file (packages/triggerdev/src/migrations-bundle.ts)
 * so that the TriggerDev migrate function can execute them without filesystem
 * access at runtime.
 *
 ...
 *
 * The generated file is NOT committed to git.  Re-run this script whenever
 * SQL migration files change and before building the TriggerDev package.
 */

import { join } from "https://deno.land/std@0.208.0/path/mod.ts";

// Apps that should NOT be bundled for TriggerDev (testing/dev-only apps)
...
async function bundleSql(): Promise<void> {
  const appsDir = "./apps";
  const outputPath = "./packages/triggerdev/src/migrations-bundle.ts";

  console.log("Bundling SQL migration files for TriggerDev...");
