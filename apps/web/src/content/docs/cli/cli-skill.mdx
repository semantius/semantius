...
The skill installs the same way as the rest of the [Agent Skills](/docs/agent-skills-installation#3-install-the-skills) bundle, so once you have the CLI on your PATH and the skills registered with your agent, `use-semantius` is already there.

If your agent supports `npx skill`, point it at the Semantius CLI repository:

<Command command="npx skill install https://github.com/semantius/semantius-cli" />

OpenClaw and other agents with their own installer pull from the same [skills folder](https://github.com/semantius/semantius-cli/tree/main/skills).
