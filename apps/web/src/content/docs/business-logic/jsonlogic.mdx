...
Semantius extends standard JsonLogic with reserved variables (`$today`, `$now`, `$user_id`, `$old`) and platform operators (`let`, `set_record`, `has_permission`, `require_permission`, `value_changed`, `concat`, `throw_error`). The extensions are scoped to the request the rule runs in and have no analog upstream. See [JsonLogic extensions](/docs/business-logic/extensions) for the full list.
