...
> Reserved variables are scoped to the record being evaluated. They are not written back to the record, they exist only for the duration of the expression.
...
### `set_record`, load a related record by id
...
### `has_permission`, check the current user's permission
...
### `require_permission`, enforce a permission
...
### `value_changed`, detect a field change on update
...
### `concat`, string concatenation with SQL semantics
...
### `throw_error`, raise a validation error
