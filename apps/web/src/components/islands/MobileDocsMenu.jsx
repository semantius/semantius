...
export default function MobileDocsMenu({ sections, groupedDocs, currentPath }) {
  ...
  // Routes are flat: /docs/<filename>, regardless of folder depth.
  // Mirrors getStaticPaths in pages/docs/[...slug].astro.
  const getRouteSlug = (id) => id.replace(/\.[^/.]+$/, "").split('/').pop();
              ...
              <nav className="flex-1 overflow-y-auto p-6">
                <div className="space-y-1">
                  {sections.map((section) => {
                    ...
                    if (section === 'root') {
                      return (
                        <ul key="root" className="space-y-1 list-none p-0 m-0">
                          {sectionDocs.map(doc => {
                            const routeSlug = getRouteSlug(doc.id);
                            const docPath = `/docs/${routeSlug}`;
                    ...
                    return (
                      <div key={section} className="space-y-1">
                        ...
                        <ul className="pl-4 space-y-1 list-none m-0">
                          {sectionDocs.map(doc => {
                            const routeSlug = getRouteSlug(doc.id);
                            const docPath = `/docs/${routeSlug}`;
                            const isExact = normalizedCurrentPath === docPath;
