...
-- =====================================================
-- TEST: unique index exists for customers_test.email
-- =====================================================

-- Test 1: Unique index was created for customers_test.email (unique_value=TRUE)
SELECT ok(
    EXISTS (
        SELECT 1 FROM pg_indexes
        WHERE tablename = 'customers_test'
          AND indexname = 'customers_test_email_unique'
    ),
    'Unique index customers_test_email_unique should exist'
);
...
SELECT lives_ok(
    $$INSERT INTO customers_test (customer_name, status, total_orders)
...
-- Test 7: Updating unique_value to FALSE on customers_test.email drops the unique index
SELECT lives_ok(
    $$UPDATE fields SET unique_value = FALSE WHERE table_name = 'customers_test' AND field_name = 'email'$$,
    'Should allow setting unique_value=FALSE on customers_test.email'
);
...
SELECT ok(
    ...
    'Unique index customers_test_email_unique should be dropped after unique_value set to FALSE'
);
