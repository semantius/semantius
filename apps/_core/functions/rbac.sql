-- =====================================================
-- RBAC SYSTEM - PL/pgSQL FUNCTIONS (CORRECTED VERSION)
-- =====================================================
...
-- Prevent circular dependencies in permission hierarchy
...
BEGIN
    ...
    IF EXISTS (
        WITH RECURSIVE hierarchy AS (
            SELECT NEW.child_permission_id AS permission_id, 1 AS depth
            UNION ALL
            SELECT ph.child_permission_id, h.depth + 1
            FROM hierarchy h
            JOIN permission_hierarchy ph ON h.permission_id = ph.parent_permission_id
            WHERE h.depth < 10
        )
        SELECT 1 FROM hierarchy WHERE permission_id = NEW.parent_permission_id
    ) THEN
        RAISE EXCEPTION 'Cannot add permission hierarchy: would create a cycle';
    END IF;
...
'Trigger function to prevent circular dependencies in permission hierarchy.';

CREATE TRIGGER check_hierarchy_cycle
    BEFORE INSERT OR UPDATE ON permission_hierarchy
    FOR EACH ROW EXECUTE FUNCTION rbac.check_permission_hierarchy_cycle();
...
BEGIN
    ...
    SELECT u.user_id, r.role_id, p_granted_by, p_expires_at
    FROM users u
    CROSS JOIN roles r
    WHERE u.external_id = p_external_id
...
'Assigns a role to a user. Optionally with expiration time.';
...
CREATE OR REPLACE FUNCTION rbac.update_updated_at()
RETURNS TRIGGER AS $$
BEGIN
...
COMMENT ON FUNCTION rbac.update_updated_at IS 
'Trigger function to automatically update updated_at timestamp.';

CREATE TRIGGER users_updated_at
    BEFORE UPDATE ON users
    FOR EACH ROW EXECUTE FUNCTION rbac.update_updated_at();
