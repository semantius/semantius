...
S4, S13, P1, B1, B3, B12, B14, R3, R4, R5, T1) mean fixed or dropped. The review,
...
- The extension is rebuilt from scratch, as a first version, once the
  ...
  the rebuild has to get right. The design of that rebuild — thin installer
  extension, `CREATE EXTENSION` plus `SELECT pg_semantius.migrate()` — is
  `plans/pg_semantius-extension-rebuild.md`.
...
| B7 | Medium | extension | `extension.ts`; `.github/workflows/extension-release.yml` | The release workflow regenerated from the tag and shipped that without proving it equals the committed, tested `extension/` and without running pgTAP. Closed on 2026-09-03 for the workflow half: it now fails on any `git status --porcelain extension/` difference (not `git diff`, which misses the generator's new upgrade scripts and pruned full installs) and runs `pg-ext-retest.sh` then `pg-cli-retest.sh` before packaging, releasing or pushing the image. | Remaining: validate META with `pgxn_meta` in the same job (B10); add `pg-ext-lifecycle.sh` once it exists; once a version is really released, fail the generator on edited or removed released migrations instead of warning (`--strict`). | The workflow cannot publish a build that differs from the committed one or that fails either suite (done); `--strict` refuses an edited released migration. |
...
| R2 | Medium | tooling | CI | No test workflow. Decision 2026-09-03: the repo uses neither pull requests nor a separate test workflow, so the suite runs inside `extension-release.yml` on the tag instead (see B7) — the first moment the artifact being published exists. Development testing stays local and uncommitted. | None as a separate workflow; the remaining CI work is tracked in B7. | Delete this row once B7's remaining items land. |
