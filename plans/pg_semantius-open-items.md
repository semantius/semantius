...
- **B5, P6, S15 and R1 were one change** and were closed together on
  ...
  trigger audits drops, which `ddl_command_end` never reports. Fresh installs
  only: `migrate()` never re-applies an applied migration, so an existing
  database keeps the old triggers.
