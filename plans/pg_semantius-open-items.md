...
| S12 | Low (DB) | migration | `0030_rbac_functions.sql` (`has_permission`, `has_any_permission`, `user_has_permission`) | `app.oauth_scopes` is a client-settable GUC (a scoped session can clear its own confinement) and the delimiter is inconsistent: comma in `has_permission`/`has_any_permission`, space in `user_has_permission`. There is no definer entry point for scopes since `set_request_context` was removed. | Store scopes inside the signed cache planned in `docs/bearer-mode-status.md`; one delimiter everywhere; a self-only `rbac.set_request_scopes(p_oauth_scopes)` with normalization and a narrow-only rule, see that document, step 5. | A scoped session that clears the GUC or calls the entry point with a wider list still has the scoped-out permission denied. |
