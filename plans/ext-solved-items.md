...
## R6 (2026-09-06): the review's scratch container is gone
...
`docker rm -f semantius-cov-scratch`, with the owner's go. It had been up for
four days on `0.0.0.0:5439` holding the release review's measurement database.
Nothing needed it: the measurement method is written out in the open items under
"How to re-measure", every number it produced is in this file, and the one
remaining reason to keep a database with `plpgsql_check` loaded - the T2
profiler repro - went away with T2 the same day.

`docker ps -a --filter name=semantius-cov-scratch` lists nothing.

---

## B18 (2026-09-06): the bundles are build output, and the code had already said so
...
**Closed with no code change: the row's premise was stale.** The row says
`packages/triggerdev/src/migrations-bundle.ts` is tracked. It is not. Commit
`f0584ae` ("feat: add release and PGXN publishing scripts") deleted it from git,
18,861 lines, and `git ls-files 'packages/*/src/migrations-bundle.ts'` returns
nothing. All three paths are ignored, the bundler's own header says "The
generated files are NOT committed to git", and `RELEASE.md` does not mention
bundles at all. The "build output" branch of the row's decision was therefore
taken, in code, on the day that commit landed; only the row had not caught up.

**The decision, recorded:** the three `migrations-bundle.ts` copies are build
output. They are generated on demand with `deno task bundle-sql`, they are never
tracked, and the release flow neither regenerates nor guards them. The reasoning
is written into `.gitignore` next to the three paths, which is where a reader
who wonders why a generated file is missing will look: the header carries a
generation timestamp, so a committed copy puts churn into every release commit
and goes stale the moment a migration changes.

`deno task bundle-sql` was run on today's tree and exits 0; `git status` shows
no bundle afterwards.

**One finding recorded rather than fixed.** All three `package.json` files list
`src/migrations-bundle.ts` under `files`, so the bundle ships inside each npm
package - and no workflow publishes those packages (`.github/workflows/` holds
`test.yml` and `extension-release.yml` only). Publishing is manual, and the
working agreement's "regenerate the `packages/*/migrations-bundle.ts` copies
before anything ships" is the only guard against publishing a stale or absent
bundle. A `prepack` script running the bundler would make that mechanical. It
changes how the packages ship, so it was left to the owner rather than done
here, and it is not tracked as a row.

---

## Q2, Q3 and Q5 (2026-09-06): accepted, the vendored pgmq findings stay
...
**Accepted. No code change, and none intended.** The owner on 2026-09-06: "did
we not decide all Q already". The decision was in fact taken on 2026-09-05 and
is recorded above under "Q1 and Q4, and the Semantius halves of Q2, Q3 and Q5":
fix every finding in our own code, keep `0160_pgmq.sql` byte-identical to
upstream v1.11.1 so the next re-vendor is a copy and not a merge, and add no
lint gate, because style warnings are not a signal worth failing a build on.
The three rows survived that day only because their Fix column left "accept and
delete, or re-vendor" open.

What is accepted: **3 + 4 + 13 = 20 findings**, every one inside the vendored
file, every one recorded above with the invocation that produced it. They are
part of the 32 `pgmq` findings the linter-context paragraph counts; the 75 grant
and `search_path` warnings on the same functions were accepted separately with
Q7.

Re-vendoring is not done for lint warnings. Upstream is at v1.12.0 (released
2026-07-14); a vendor bump is its own piece of work with its own verification,
and it is the natural moment to re-lint the file. That is the trigger for
revisiting this, and the only one.

**Q6 is the only Q row left**, and it is not about pgmq: seven Semantius trigger
functions the linter never sees, six of them because plpgsql_check 2.10 stops at
`relation "new_rows" does not exist` when no transition table is declared.

---

## T2 (2026-09-06): dropped, the profiler bug is not ours to report
...
**Dropped by the owner on 2026-09-06: "T2 is not our problem".** No code
changed. The bug is worked around in `0145_managed_enable.sql`, where the
comment explains the workaround on its own terms - build the statement into a
variable, then `EXECUTE` the variable, because the profiler re-evaluates the
string expression after the statement has run - and names no plan, no row and no
appendix. Nothing in the tree depends on this row existing.

Searched for an existing upstream report on 2026-09-06 (`profiler EXECUTE
queryid`, `dyn_queryid`, `profiler regprocedure`): no match. So the draft below
is not superseded by anything. It is carried here in full, verbatim from the
open items' Appendix A, which was deleted with the row; anyone who later wants
to file it has the complete text and the repro.
...
The open items no longer reference Appendix A anywhere; the T2 row was its only
reader, and the appendix went with it.

---

## B17 (2026-09-06): the generator deletes upgrade scripts the manifest does not know
...
**Both of the row's alternatives are now taken.** The second landed on
2026-09-04: `scripts/archive-manifest.sh` derives the archive's file set from
`extension/versions.json` and the release workflow calls it with `--check`, so
an orphan fails the release instead of shipping. The first is this change, and
it is what the row's "deleted by the next generation" clause asked for.

What the archive-only fix left open, and why it mattered: the prune kept every
file with a second `--` forever, so an orphan sat in `extension/` on every
developer's disk until someone noticed - and both Dockerfiles copy the SQL by
glob (`docker-postgres/Dockerfile:28`, `pgdocker/Dockerfile.ext:16`). The
release path's `--check` runs before its image build and would abort the run,
but a local `docker-postgres/build.sh` is not gated and would bake the orphan
into an image.

What changed:

- `packages/cli/commands/extension.ts`: `pruneOldFullInstalls` became
  `pruneStaleScripts(dir, name, keepVersion, knownVersions)`. A file
  `<name>--<mid>.sql` is kept when `<mid>` splits on `--` into one part equal to
  `keepVersion` (the current full install) or into exactly two parts that are
  both manifest versions (a live link of the upgrade chain). Everything else is
  removed. The whole chain is kept, not only the links ending at `keepVersion`,
  because `make install` needs all of it.
- The caller runs after `manifest.versions[version] = ...`, so the version being
  generated is already in `new Set(Object.keys(manifest.versions))` and a
  freshly written upgrade script is never pruned by the run that wrote it.
- Three comments described the old behavior and now describe this one: the
  frozen-version guard in `extension.ts`, the header of
  `scripts/archive-manifest.sh`, and the packaging step of
  `extension-release.yml`. The archive check is the second guard, not the only
  one, and both files say so.

**Pinned by `scripts/check-prune-orphans.sh`**, called from `test.yml` right
after the regeneration step, so it runs on every pull request. There is no
`deno test` anywhere in this repository - no `*_test.ts` exists and CI runs
`deno task check` - and introducing a unit-test runner for one function was out
of proportion, so the pin is a shell check: copy `extension/` into a temp
directory, drop in two fabricated orphans (`pg_semantius--0.4.0--<v>.sql`, whose
`<from>` is unknown, and `pg_semantius--<v>--9.9.9.sql`, whose `<to>` is), run
`deno task extension <v> --output <tmp>`, and assert both are gone while the
full install, the control file and the manifest are still there. The copy brings
`versions.json`, so the frozen-version guard accepts the target as the newest
build. Every write path in the generator derives from `outputDir` (checked line
by line), so the real `extension/` is never touched - confirmed by
`git status --porcelain -- extension/` being empty after a run.

**Failing-capable, verified both ways.** Against the previous `extension.ts` the
script reports both orphans as survivors and exits 1; against the new one it
exits 0. `deno task check` is clean. `deno task extension 0.5.0-beta1` on the
real `extension/` produced only the three migration-hash changes that R10's
comment edits caused - no prune-driven diff, because the directory holds no
orphan today.

**Not done, deliberately:** the two Dockerfile globs stay globs. They copy from
a directory the generator has just cleaned, a version bump needs no edit in
them, and `--check` covers the archive on the release path. The keep-case - an
upgrade script whose endpoints ARE in the manifest surviving the prune - cannot
be exercised while `versions.json` holds a single version; it becomes testable
with **R9**, which fabricates a second one.

---

## R10 (2026-09-06): the policy-form test is named for what it checks

The row as it stood in the open items:

| ID | Priority | Area | Where | Problem | Fix | Done when |
|---|---|---|---|---|---|---|
| R10 | Low | tooling | `apps/test/tests/0445_test_policy_initplan_form.sql` | The test is named for the InitPlan form and reads as a guarantee that a policy's permission check runs once per statement. It asserts something weaker: that the call appears inside a sub-select. `modules_select_policy` (`0050_rbac_rls.sql:47`) is `(select rbac.has_any_permission('admin', view_permission))` - the column reference makes the sub-select correlated, so it is a SubPlan evaluated per row - and it passes. The policy itself is accepted (under 20 modules, about 0.5 ms warm); the test claiming to cover it is the problem. | Either rename the test to what it checks, or make it distinguish a correlated sub-select from an uncorrelated one - `EXPLAIN` of a policy-guarded query and a look for `SubPlan` against `InitPlan` is the direct form. | The test's name matches what it asserts, and a deliberately correlated policy added as a fixture fails it. |

**Closed on the rename**, the owner choosing between the row's two alternatives
on 2026-09-06. The row's second done-when clause - a correlated fixture that
fails the test - is deliberately not delivered, and the reason is that the
discriminator would guard almost nothing: every policy the generators emit
interpolates the permission name as a literal through `%L`, so a column
reference cannot appear inside a generated sub-select, and the only correlated
policy in the catalog is the hand-written `modules_select_policy`, accepted at
its size (under twenty modules, about 0.5 ms warm). With the file saying plainly
what it checks and what it does not, nothing is left overpromising.
...
- The header is rewritten. It states the rule (the call goes inside a scalar
  ...
  its own: an uncorrelated sub-select becomes an InitPlan run once per
  statement, a correlated one stays a SubPlan run per row at the cost of the
  bare form, this file checks the form and both pass, and telling them apart
  needs `EXPLAIN` and a look for `SubPlan` against `InitPlan`. It then names
  `modules_select_policy` as the one correlated policy in the catalog and says
  it is accepted. The detection paragraphs are unchanged; they were correct.
- The five assertion descriptions that said "in the InitPlan form" say "in the
  sub-select form", which is what each of them counts. What is asserted did not
  change: `plan(14)` and every predicate are untouched.
- Four comments elsewhere referred to it. `0180_computed_validation.sql:316`
  named the file and uses the new name. The three saying "test 0445 fails on the
  bare per-row form" stay, because that remains true; two of them also carried a
  bare `(P1)` tracking id, which points at a row closed long ago and explains
  nothing, and that is gone.
- `deno task extension 0.5.0-beta1` regenerated the build: the comment edits
  moved the hashes of `0070_dd_functions`, `0145_managed_enable` and
  `0180_computed_validation`.

Nothing in the tree refers to the old name any more, outside this record and
the open items bullet that quotes it. Both harnesses green with the renamed
file in the run list, 89 files and 2,287 passing on each
(`pg-cli-retest.sh` and `pg-ext-retest.sh`, 2026-09-06).

---

## S19 (2026-09-07): the ownership transfer no longer depends on catalog order

The row as it stood in the open items:

| ID | Priority | Area | Where | Problem | Fix | Done when |
|---|---|---|---|---|---|---|
| S19 | Low | migration | `0290_owner_hardening.sql` (the `pg_proc` ownership loop) | The loop that runs `ALTER FUNCTION ... OWNER TO semantius_owner` has no `ORDER BY`, and the DDL audit event trigger fires on every statement it issues. `audit.log_ddl_event()` is SECURITY DEFINER and calls `audit.current_user_id()`, which 0150 revokes from PUBLIC: the moment `log_ddl_event` changes owner it starts running as `semantius_owner`, and if `current_user_id` has not moved yet the migration dies on `log_ddl_event`'s own ALTER with `permission denied for function current_user_id`. Which of the two moves first is `pg_proc` heap order, so adding or removing any function anywhere in the codebase can flip it - it flipped on 2026-09-04 when P11 added three, and a clean tree migrated fine minutes earlier. Fixed by granting EXECUTE on `audit.current_user_id()` to `semantius_owner` before the loop, which grants nothing the end state lacks since that role owns the function seconds later. | Residue: the loop is still order-dependent for any future SECURITY DEFINER function that calls a PUBLIC-revoked helper during DDL, and nothing detects it until an install fails. Either order the loop explicitly, or remove the event trigger's dependency on a revoked function. **Owned by `plans/2026-09-06-2219-s19-owner-loop-pregrant.md`.** | A new SECURITY DEFINER function calling a PUBLIC-revoked helper cannot break 0290, by test or by construction. |

**Closed by construction**, which is what the row's done-when allows. The single
`GRANT EXECUTE ON FUNCTION audit.current_user_id()` that named one pair of
functions is replaced by a loop that runs the ownership query once more, before
anything moves, and grants `semantius_owner` EXECUTE on every function it is
about to move **that the role cannot already execute**. Whatever the audit
trigger reaches for mid-transfer, in whatever order the catalog returns the
functions, it may call it. Two passes over one query is deliberate: every grant
has to exist before the first owner changes, so a single pass cannot do it.

### The one condition, and why it is there

The plan this closure carried out specified the grant unconditionally, and its
own gate was that the finished catalog must be unchanged. It was not: 75
functions - every vendored `pgmq` one - carry no ACL of their own, which means
PUBLIC may execute them, and granting the owner a right it already held wrote
out an explicit ACL where there had been none. The privileges were the same
either way, exactly `acldefault('f', 'semantius_owner')`, and `pg_dump` emitted
nothing extra, but it was a catalog difference for no gain. The owner chose the
narrower form on 2026-09-07 after the trade-off was put with both measured:

```
AND NOT has_function_privilege('semantius_owner', p.oid, 'EXECUTE')
```

Every function this leaves in the set already carries an explicit ACL, and there
the grant dissolves on its own: `ALTER ... OWNER TO` rewrites every entry where
the old owner is grantee or grantor onto the new owner and merges duplicates, so
`{installer=X/installer, semantius_owner=X/installer}` ends as
`{semantius_owner=X/semantius_owner}` - what it ends as without the pre-grant.

### Proof

- **The end state is byte-identical.** A fresh `_core` migrate from the tree
  before the change and from the tree after it, compared over
  `p.oid::regprocedure, p.proowner, p.proacl` for all 320 functions in the five
  Semantius schemas: **empty diff**. The unconditional form differed on 75 rows;
  this one differs on none.
- **`pg_dump` is byte-identical.** Lifecycle step 10 (the migrate-installed and
  extension-installed schemas compared line by line): 23,619 lines each,
  identical.
- **The order-independence check, forced both ways.** `ORDER BY (p.proname =
  'log_ddl_event') DESC, p.oid` on the ownership loop puts the audit function
  first, the order that fails. With no pre-grant at all, a fresh CLI install
  dies exactly as the row describes:

  ```
  Message: permission denied for function current_user_id
  Code: 42501
  Where: PL/pgSQL function audit.log_ddl_event() line 6 at assignment
  SQL statement "ALTER FUNCTION audit.log_ddl_event() OWNER TO semantius_owner"
  ```

  With the pre-grant loop in place and the same forced order, `_core,nwind,test`
  installs green. The `ORDER BY` was reverted; it exists nowhere in the tree.
  There is no permanent test for this, and there should not be: the property is
  structural, and a test that reordered a migration's loop would be testing a
...
Step 10 compares the two containers' schema dumps and will fail with four lines
of difference - `CREATE SCHEMA extensions` and `CREATE EXTENSION plpgsql_check`
- if the CLI database has had a coverage run and the extension database has not.
`deno task test --coverage` installs `plpgsql_check` into an `extensions` schema
