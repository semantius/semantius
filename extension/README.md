...
```sh
pgxn install pg_semantic_platform
```
...
cp pg_semantic_platform.control pg_semantic_platform--*.sql "$(pg_config --sharedir)/extension/"
```

**Then, in the target database:**

```sql
CREATE EXTENSION pg_semantic_platform CASCADE;        -- fresh install of the current version
```

**Upgrade** an existing install to a newer version (walks the bundled
`pg_semantic_platform--<old>--<new>.sql` chain):

```sql
ALTER EXTENSION pg_semantic_platform UPDATE;          -- or UPDATE TO '<version>'
```

## Build a new version

Run from the repo root. Writes the current full install `pg_semantic_platform--<version>.sql`,
an upgrade script `pg_semantic_platform--<prev>--<version>.sql` containing the migrations added
since the previous version, bumps `default_version` in `pg_semantic_platform.control`, records
...
`DROP EXTENSION pg_semantic_platform` will drop the data-dictionary tables **and their
