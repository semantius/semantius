...
cp semantius.control semantius--*.sql "$(pg_config --sharedir)/extension/"
```

**Then, in the target database:**

```sql
CREATE EXTENSION semantius CASCADE;        -- fresh install of the current version
```

**Upgrade** an existing install to a newer version (walks the bundled
`semantius--<old>--<new>.sql` chain):

```sql
ALTER EXTENSION semantius UPDATE;          -- or UPDATE TO '<version>'
```

## Build a new version

Run from the repo root. Writes the current full install `semantius--<version>.sql`,
an upgrade script `semantius--<prev>--<version>.sql` containing the migrations added
since the previous version, bumps `default_version` in `semantius.control`, records
...
`DROP EXTENSION semantius` will drop the data-dictionary tables **and their
