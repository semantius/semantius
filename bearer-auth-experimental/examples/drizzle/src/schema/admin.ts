...
export const auditRecordLogs = pgTable("audit_record_logs", {
  ...
  clientAddr: text("client_addr").notNull(),
...
export const fields = pgTable("fields", {
  ...
  format: text("format", { enum: ["json", "html", "text", "multiline", "code", "jsonata", "jsonlogic", "reference", "parent", "enum", "date", "time", "date-time", "duration", "uri", "uri-reference", "iri", "iri-reference", "uri-template", "url", "email", "idn-email", "hostname", "idn-hostname", "ipv4", "ipv6", "regex", "uuid", "json-pointer", "json-pointer-uri-fragment", "relative-json-pointer", "byte", "binary", "password", "int32", "int64", "float", "double", "string", "number", "integer", "boolean", "object", "array"] }).notNull().default("text"),
...
export const modules = pgTable("modules", {
  ...
  description: text("description").notNull(),
  moduleType: text("module_type", { enum: ["domain", "master", ""] }).notNull().default("domain"),
  viewPermission: text("view_permission").references((): AnyPgColumn => permissions.permissionName),
  ...
  defaultManagerRoleId: bigint("default_manager_role_id", { mode: "number" }).references((): AnyPgColumn => roles.id, { onDelete: "set null" }),
  defaultAdminRoleId: bigint("default_admin_role_id", { mode: "number" }).references((): AnyPgColumn => roles.id, { onDelete: "set null" }),
  catalogModuleCode: text("catalog_module_code").notNull(),
  domainCode: text("domain_code").notNull(),
  accessScope: text("access_scope", { enum: ["basic", "full"] }).notNull().default("basic"),
...
export const permissionHierarchy = pgTable("permission_hierarchy", {
  ...
  origin: text("origin", { enum: ["system", "model", "model_master", "user", ""] }).notNull().default("user"),
  createdAt: timestamp("created_at", { withTimezone: true }).defaultNow(),
});
...
export const queueTableEvents = pgTable("queue_table_events", {
  queueId: bigint("queue_id", { mode: "number" }).references((): AnyPgColumn => queues.id, { onDelete: "cascade" }).notNull(),
  id: bigint("id", { mode: "number" }).primaryKey().generatedByDefaultAsIdentity(),
...
export const raciAssignments = pgTable("raci_assignments", {
  ...
  id: bigint("id", { mode: "number" }).primaryKey().generatedByDefaultAsIdentity(),
  processId: bigint("process_id", { mode: "number" }).references((): AnyPgColumn => processes.id, { onDelete: "cascade" }).notNull(),
  roleId: bigint("role_id", { mode: "number" }).references((): AnyPgColumn => roles.id, { onDelete: "cascade" }),
...
export const raciEvents = pgTable("raci_events", {
  ...
  targetRoleId: bigint("target_role_id", { mode: "number" }).references((): AnyPgColumn => roles.id, { onDelete: "cascade" }),
...
export const rolePermissions = pgTable("role_permissions", {
  id: text("id").primaryKey(),
  roleId: bigint("role_id", { mode: "number" }).references((): AnyPgColumn => roles.id, { onDelete: "cascade" }).notNull(),
  permissionName: text("permission_name").references((): AnyPgColumn => permissions.permissionName, { onDelete: "cascade" }).notNull(),
  grantedAt: timestamp("granted_at", { withTimezone: true }),
  grantedBy: bigint("granted_by", { mode: "number" }).references((): AnyPgColumn => users.id, { onDelete: "set null" }),
});
...
export const userPermissions = pgTable("user_permissions", {
  id: text("id").primaryKey(),
  userId: bigint("user_id", { mode: "number" }).references((): AnyPgColumn => users.id, { onDelete: "cascade" }).notNull(),
  permissionName: text("permission_name").references((): AnyPgColumn => permissions.permissionName, { onDelete: "cascade" }).notNull(),
  grantedAt: timestamp("granted_at", { withTimezone: true }),
  grantedBy: bigint("granted_by", { mode: "number" }).references((): AnyPgColumn => users.id, { onDelete: "set null" }),
});
...
export const userRoles = pgTable("user_roles", {
  ...
  assignedBy: bigint("assigned_by", { mode: "number" }).references((): AnyPgColumn => users.id, { onDelete: "set null" }),
});
...
export const webhookReceiverLogs = pgTable("webhook_receiver_logs", {
  webhookReceiverId: bigint("webhook_receiver_id", { mode: "number" }).references((): AnyPgColumn => webhookReceivers.id, { onDelete: "cascade" }).notNull(),
  id: bigint("id", { mode: "number" }).primaryKey().generatedByDefaultAsIdentity(),
  ...
  result: text("result", { enum: ["10", "20", "30", "40", "50", "60", ""] }).notNull().default("10"),
...
export const webhookReceiversRelations = relations(webhookReceivers, ({ one, many }) => ({
  ...
  webhookReceiverLogs: many(webhookReceiverLogs, { relationName: "webhook_receiver_logs_webhook_receiver_id" }),
}));
