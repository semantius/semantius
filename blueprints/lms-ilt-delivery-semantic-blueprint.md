...
| --- | --- | --- |
| Course Offerings | `course_offerings` | Scheduled instance of a course: date range, location, capacity, language, delivery mode. |
| Course Resources | `course_resources` | Handout, supplementary material, or pre-read attached to an offering or session. |
| Course Sessions | `course_sessions` | Individual session within a multi-day offering; carries per-session attendance and instructor assignment. |
| Instructor Assignments | `instructor_assignments` | Junction assigning an instructor to a session or offering, with role (primary / assistant / observer). |
| Instructors | `instructors` | Instructor master separate from employees; covers internal trainers, contracted SMEs, and external vendor instructors. |
| Session Attendances | `session_attendances` | Per-session attendance roster row per learner. OSHA-mandated evidence for required workplace training. |
| Session Cancellations | `session_cancellations` | Cancellation or no-show record for an ILT session, driving waitlist promotion. |
| Session Rosters | `session_rosters` | Pre-session enrollment snapshot for an ILT session; attendance proof basis. |
| Training Room Bookings | `training_room_bookings` | Reservation of a physical or virtual room and resources for an instructor-led session. |
