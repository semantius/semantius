...
| Onboarding Document Collections | `onboarding_document_collections` | Record of paperwork collected from a new hire as part of the journey: I-9, NDA, handbook acknowledgment, direct-deposit form, country-specific tax forms, country-specific work-permit evidence. Captures signature state, external e-signature envelope reference ids, and audit metadata. Distinct from the canonical document templates, which would be mastered by a Document Management domain (not yet in catalog). |
...
| Project Resource Allocations | `project_resource_allocations` | Forward-looking resource commitment plan (skill, planned utilization %, effective period) - distinct from executed assignments. |
| Resource Skill Inventories | `resource_skill_inventories` | Pool of skills / competencies across the resource base with proficiency levels - anchors PSA utilization forecasting. |
