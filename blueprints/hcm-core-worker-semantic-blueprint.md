...
| --- | --- | --- |
| Emergency Contacts | `emergency_contacts` | People to contact in an emergency for a worker, with priority ordering and relationship. Holds personal data on third parties beyond the employee. |
| Employee Dependents | `employee_dependents` | Spouse, children, and other dependents of a worker, held as person-level records on the worker. Benefits administration reads these for eligibility and beneficiary designation; the plan-enrollment record is held separately by benefits. |
...
| National IDs | `national_ids` | Government-issued identity numbers for a worker, such as social security or national insurance numbers, held with restricted access and validity dates. Jurisdiction-specific. |
| Work Eligibility Documents | `work_eligibility_documents` | Right-to-work evidence for a worker: visa, work permit, or equivalent authorization, effective-dated with an expiry the system tracks. Jurisdiction-conditional and access-restricted. Distinct from the US-specific hiring I-9 record. |
| Worker Addresses | `worker_addresses` | Home, work, and mailing addresses for a worker, effective-dated and typed by purpose, supporting multi-jurisdiction and country-variant formats. |
| Locations | `locations` | - |
...
| Contingent Workers | `contingent_workers` | Non-employee resource: 1099 contractor, agency staff, statement-of-work consultant, intern. Carries supplier, role, rate, end date, classification basis. Distinct from employees by lifecycle, payment path, and benefits eligibility. |
| MDM Source Records | `source_records` | Raw record from a source system before match/merge processing; preserves provenance and source identifier for lineage. |
| Onboarding Document Collections | `onboarding_document_collections` | Record of paperwork collected from a new hire as part of the journey: I-9, NDA, handbook acknowledgement, direct-deposit form, country-specific tax forms, country-specific work-permit evidence. Captures signature state, external e-signature envelope reference ids, and audit metadata. Distinct from the canonical document templates, which would be mastered by a Document Management domain (not yet in catalog). |
| Pay Slips | `pay_slips` | Per-employee per-cycle gross-to-net statement: earnings, pre/post-tax deductions, employer contributions, tax withholdings, net pay, YTD totals. Called Pay Stub (US), Payslip (UK/AU/IE), Bulletin de Paie (FR), Lohnzettel (DE), Holerite (BR). Worker-portal accessible; legally retained per jurisdiction. |
| People KPIs | `people_kpis` | Calculated workforce KPIs and metrics: time-to-fill, attrition rate, retention rate, span-of-control, internal-mobility rate, training completion rate, cost-per-hire, time-to-productivity. Derived from HCM, ATS, LMS, Payroll event streams. PA is the master of the calculated KPI definitions and the materialized metric values; source systems master the underlying events. |
...
| Project Resource Allocations | `project_resource_allocations` | Forward-looking resource commitment plan (skill, planned utilisation %, effective period) - distinct from executed assignments. |
| Resource Skill Inventories | `resource_skill_inventories` | Pool of skills / competencies across the resource base with proficiency levels - anchors PSA utilisation forecasting. |
