...
| --- | --- | --- |
| Worker Change Requests | `worker_change_requests` | Self-service request raised by an employee or manager to change worker data (personal details, address, job, or org assignment), routed through an approval workflow before it commits to the worker record. The self-service surface's first-class request object. |
...
| Asset Lifecycle Events | `asset_lifecycle_events` | Cross-cutting lifecycle audit log applicable to hardware, software, and SaaS assets: procurement, deployment, transfer, retirement, disposal. ITSM (incident-driven failure events), HCM (employee termination → recall), and Onboarding (provisioning) all contribute events. |
| Absence Balances | `absence_balances` | Per-worker per-leave-type running balance: accrued, taken, scheduled, available. Calculated against time_off_policies and reset per accrual cycle. |
| Absence Requests | `absence_requests` | Worker-initiated request for time away: PTO, sick, parental, bereavement, jury duty, FMLA. Carries policy, eligibility check, approval chain, and accrual deduction. Called Leave Request (UK), Vacation Request (US), PTO Request. |
...
| Compensation Plans | `compensation_plans` | Total-rewards package definition per role, level, location: base structure, bonus target, equity guideline, allowances. Distinct from sales-incentive plans (those live in Sales Performance Management). |
