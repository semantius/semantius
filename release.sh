...
die() { echo "release.sh: $*" >&2; FINISHED=1; exit 1; }

# A release script that stops without saying so is the worst thing it can do:
# the build looks finished, nothing is tagged, and CI never runs. `die` reports
# itself, but `set -e` does not - it just exits. This trap catches every other
# way out and says where it got to, so an unexplained stop can never look like
# success again.
FINISHED=0
on_exit() {
  local rc=$?
  ...
  [ "$rc" = "0" ] && return 0
  echo >&2
  echo "release.sh: STOPPED at \"${STAGE:-startup}\" (exit $rc) without completing." >&2
  echo "  Nothing was tagged or pushed unless a line above says otherwise." >&2
  if [ -n "${RELEASE_COMMIT:-}" ]; then
    echo "  The build IS committed as $(git rev-parse --short HEAD 2>/dev/null)." >&2
    echo "  Re-run ./release.sh ${TARGET:-<version>} to finish, or undo with:" >&2
...
if [ -n "$BUILT" ]; then
  for v in $(jq -r '.versions | keys_unsorted[]' "$MANIFEST" 2>/dev/null | tr -d '\r'); do
    # Strictly above: the target's own entry is what we are about to rewrite.
    if [ "$(semver_cmp "$v" "$TARGET")" != "1" ]; then continue; fi
    if git rev-parse -q --verify "refs/tags/v$v" >/dev/null 2>&1; then continue; fi
...
# ==========================================================================
NEW_VERSION_NOTE=""
if [ "$MODE" = "new" ] && [ -n "$BUILT" ]; then
  NEW_VERSION_NOTE=", write the $BUILT -> $TARGET upgrade script and prune the $BUILT full install"
fi
...
if git diff --cached --quiet; then
  ...
  # Built into a variable, not a `{ ...; } | git commit` pipeline. A group whose
  # last command is a false AND-list exits 1, pipefail propagates that, and
  # set -e then kills the script AFTER the commit has already been made - which
  # is exactly how a release once ended with a commit, no tag and no error.
  COMMIT_MSG="$SUBJECT
"
  COMMIT_MSG="$COMMIT_MSG
Generated by ./release.sh $TARGET. Full install $NAME--$TARGET.sql;
default_version $TARGET."
  if [ "$ALLOW_EDITED" = "1" ]; then
    COMMIT_MSG="$COMMIT_MSG

Allow-edited-migrations: yes"
  fi
...
if [ "$NOTHING_CHANGED" = "1" ]; then
  # Nothing to commit is NORMAL when finishing an interrupted release: the build
  # was committed by the run that stopped, and this run only has to tag and push
  # it. It is only wrong if the build for this version is genuinely absent - so
  # check for the artifacts rather than assuming a new version must produce a
  # diff, which is what used to make the recovery run die.
  [ -f "$EXT_DIR/$NAME--$TARGET.sql" ]     || die "internal: nothing to commit, but $EXT_DIR/$NAME--$TARGET.sql does not
  exist. The build for $TARGET was not produced; nothing was tagged."
  jq -e --arg v "$TARGET" '.versions | has($v)' "$MANIFEST" >/dev/null 2>&1     || die "internal: nothing to commit, but $MANIFEST does not list $TARGET.
  The build for $TARGET was not produced; nothing was tagged."
  echo "  the build for $TARGET is already committed - continuing to tag it"
fi
