...
REM   3. readiness gate poll until the semantius extension is present.
REM   4. migrate --apps cloud,test,nwind   migrate auto-prepends _core, SKIPPED
REM                     because the extension seeded _versions; deploys only
REM                     cloud,test,nwind (cloud is required: test seeds into the
REM                     webhook_receivers table that the cloud app creates).
...
:wait
set "EXTOK="
for /f "usebackq delims=" %%R in (`docker exec %CONTAINER% psql -U postgres -d %POSTGRES_DB% -tAc "SELECT 1 FROM pg_extension WHERE extname='semantius'" 2^>nul`) do set "EXTOK=%%R"
if "%EXTOK%"=="1" goto :ready
set /a tries+=1
if %tries% geq 90 (
  echo Timed out waiting for the semantius extension to install.
...
pushd "%REPO_ROOT%"
call deno task migrate --apps cloud,test,nwind --database-url "%EXT_URL%" || (popd & goto :err)
