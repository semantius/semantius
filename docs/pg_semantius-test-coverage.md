...
| `rbac.set_request_context(text, text, text)` | 0190:187 | session-mode entry point |
| `public.ping()` | 0080:608 | RPC without `rbac.uid()` |
| `public.queue_pop(text)`, `queue_archive(text,bigint)`, `queue_delete(text,bigint)` | 0170:382-437 | SECURITY DEFINER queue mutators |
...
- Extension lifecycle: `ALTER EXTENSION ... UPDATE`, `DROP EXTENSION`,
  `pg_dump`/`pg_restore`, second database on one cluster, non-superuser
  install, `CREATE EXTENSION ... SCHEMA x` (covered by the packaging review,
  not by pgTAP).
...
| `0415_test_queue_rpc_mutators.sql` | `queue_pop`, `queue_archive`, `queue_delete` (admin path) |
| `0420_test_audit_truncate.sql` | `audit.truncate_trigger` |

Still never called after these: `rbac.set_request_context` (deliberately not
pinned; see review finding on impersonation), 37 pgmq functions
...
- `rbac.set_request_context(p_external_id text, p_email text, p_oauth_scopes text)` [definer] — 0190_user_name_claims.sql:187
