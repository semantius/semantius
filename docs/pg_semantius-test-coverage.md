...
| 2026-09-05 | 192/302 (63.6%) | 1,870/2,225 (84.0%) | 2,175 | first measurement since 2026-09-03, so it spans four changes, not one: P11 (statement-level searchable triggers), P3, P12, and P13/P4 (statement-level audit and queue, the request context hoisted to an InitPlan). Only the last of those is separable here - it added `0447_test_request_context.sql` (16) and `0448_test_statement_triggers.sql` (25), extended `0445` by 2 and `0300` by 1, and put three new functions into the universe (`public.jl_request_context`, `audit.insert_trigger`, `audit.delete_trigger`), all of them called. Measured on the migrate path (`pgdocker/pg-cli-retest.sh`), not the extension path, because the shared dev database was being written to concurrently by an external client at the time. |
