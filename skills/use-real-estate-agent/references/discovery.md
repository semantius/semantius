...
1. **Deploy stamp (`settings.domain_code`)**, the deploy pipeline writes
   ...
   `settings` JSONB. Querying `settings->>domain_code = REAL-ESTATE-AGENT` resolves the slice directly and is
...
semantius call crud postgrestRequest '{"method":"GET","path":"/modules?settings->>domain_code=eq.REAL-ESTATE-AGENT&select=id,module_slug,module_name,catalog_module_code,settings"}'
...
```
Real Estate Agent (solo / small firm bundle) discovery complete (deterministic).
