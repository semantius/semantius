...
| Domain not deployed (empty slice) | Stop; offer the blueprint deploy. | *"The Real Estate Brokerage and Agent Operations domain isn't deployed on your platform yet. It can be added from the catalog. Want me to walk you through deploying it?"* |
...
1. **Verify `use-semantius` is loaded in the session.** Look at the available-skills list in the current system reminder and confirm it is genuinely absent (do not assume from a prior turn). If it is not present: first say this exact message (use the command below verbatim; do NOT invent an install URL): *"This skill delegates all Semantius CLI mechanics to `use-semantius`. Install it and reload the session: `npx skills add https://github.com/semantius/semantius-cli/skills/use-semantius`"*; then **offer to run that command for the user** (ask first, it modifies their setup; never install silently). Because an added skill only loads after a session reload, which the agent cannot trigger, ask the user to reload and do not proceed until `use-semantius` appears in the skills list. This is an agent-level check (no script can see what's loaded in the Claude Code session); the agent performs it on every invocation as the cheapest gate.
