...
| **Domain not deployed (empty slice)** | ⛔ **HARD STOP. Deliver the message and stop. No further action.** Do NOT call `semantius-admin` (or any skill) to deploy it yourself. Do NOT try to deploy, install, or provision the domain. | *"The HVAC Service Management (small-org starter) domain isn't deployed on your platform yet. It can be added from the catalog: https://www.semantius.com/blueprints/hvac-svc-mgmt/"* |
