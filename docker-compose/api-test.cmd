...
REM   1. the Caddy front door: SPA, runtime config, /rest/, /api-docs/
...
curl -s -o NUL -w "   /rest/         HTTP %%{http_code}  (spec through the front door)\n" http://localhost:3000/rest/
curl -s -o NUL -w "   /gateway/rest/ HTTP %%{http_code}  (spec through the idp gateway)\n" http://localhost:3000/gateway/rest/
